...
export default function AdminDemandesPage() {
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (demande) =>
          ...
          demande.stagiaires?.users?.email.toLowerCase().includes(searchQuery.toLowerCase()),
      )
    }
  ...
  const handleQuickAction = async (demandeId: string, action: "approve" | "reject") => {
    try {
      const newStatus = action === "approve" ? "approuvee" : "rejetee"
      const { error } = await supabase
        .from("demandes")
        .update({
          statut: newStatus,
          date_reponse: new Date().toISOString(),
          commentaire_reponse: action === "approve" ? "Approuvé par l'administrateur" : "Rejeté par l'administrateur",
        })
        .eq("id", demandeId)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Superviser toutes les demandes de la plateforme</p>
        </div>
                        ...
                        {demande.statut === "en_attente" && (
                          <>
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleQuickAction(demande.id, "approve")}
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleQuickAction(demande.id, "reject")}
